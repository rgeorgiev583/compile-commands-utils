...
def get_include_paths_from_compile_commands(path: str):
    ...
    try:
        with open(compile_commands_path, 'r') as compile_commands_file:
            compile_db_json = compile_commands_file.read()
    except OSError:
        print(format('error: file {} does not exist', compile_commands_path))
        return

    compile_commands = json.loads(compile_db_json)
    argparser = argparse.ArgumentParser()
    argparser.add_argument('-I', action='append', default=[], dest='include_paths')
